...
tryCatch({
  ...
  writeLines("success", file.path(OUTPUT_DIR, "Figure3_status.txt"))
  message(
  ...
  figure3_error <<- e
  calls <- vapply(sys.calls(), function(x) paste(deparse(x), collapse = " "), character(1))
  writeLines(
    c(
      "Figure 3 execution failed.",
      paste0("Error: ", conditionMessage(e)),
      "",
      "Call stack:",
      calls
    ),
    file.path("output", "Figure3_ERROR.txt")
  )
  message("Figure 3 execution failed: ", conditionMessage(e))
})

if (!is.null(figure3_error)) {
  message("Failure details were saved to output/Figure3_ERROR.txt")
}
